...
class SimpleResumeApp:
    ...
    def render_home(self):
        ...
        feature_card('fas fa-robot', 'AI-Powered Analysis', 'Instant feedback to optimize your resume for ATS and roles.')
        feature_card('fas fa-magic', 'Resume Builder', 'Create and download polished resumes.')
        feature_card('fas fa-chart-line', 'Career Insights', 'Data-driven insights to improve your job search.')
        ...
        try:
            ...
            class SimpleResumeApp:
                ...
                def render_analyzer(self):
                    apply_modern_styles()
                    page_header('Resume Analyzer', 'Upload a resume (PDF or DOCX) to get basic AI-powered feedback')

                    uploaded = st.file_uploader('Upload resume', type=['pdf', 'docx', 'txt'])
                    if uploaded is None:
                        st.info('Upload a resume to analyze.')
                        return
                    ...
                    # Basic formatting and section checks
                    format_score, deductions = self.analyzer.check_formatting(text)
                    st.metric('Formatting Score', f'{int(format_score)} / 100')
                    if deductions:
                        with st.expander('Formatting Issues'):
                            for d in deductions:
                                st.write('- ' + d)

                    # Extract skills, summary, experience counts
                    skills = self.analyzer.extract_skills(text)
                    experiences = self.analyzer.extract_experience(text)
                    education = self.analyzer.extract_education(text)

                    st.markdown('### Extracted Information')
                    st.write('Skills:', ', '.join(skills) if skills else 'None found')
                    st.write('Experience blocks found:', len(experiences))
                    st.write('Education entries found:', len(education))

                    # Simple downloadable cleaned text
                    if st.button('Download extracted text'):
                        st.download_button('Download', text, file_name='extracted_resume.txt', mime='text/plain')

                def render_job_search(self):
                    try:
                        render_job_search()
                    except Exception as e:
                        st.error(f'Error rendering job search: {e}')

                def main(self):
                    st.sidebar.title('Navigation')
                    choice = st.sidebar.radio('Go to', ['Home', 'Resume Analyzer', 'Job Search'])
                    mapping = {'Home': 'home', 'Resume Analyzer': 'analyzer', 'Job Search': 'job_search'}
                    st.session_state.page = mapping.get(choice, 'home')

                    # Render selected page
                    page = st.session_state.get('page', 'home')
                    func = self.pages.get(page, self.render_home)
                    func()
